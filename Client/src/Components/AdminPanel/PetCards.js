...
const PetCards = (props) => {
  ...
  const handleApprove = async () => {
    ...
    try {
      const response = await fetch(
        `https://paws-for-u-backend.onrender.com/approving/${props.pet._id}`,
        {
          method: "PUT",
          body: JSON.stringify({
            status: "Approved",
  ...
  const deleteFormsAdoptedPet = async () => {
    ...
    try {
      const deleteResponses = await fetch(
        `https://paws-for-u-backend.onrender.com/form/delete/many/${props.pet._id}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleReject = async () => {
    try {
      const response = await fetch(
        `https://paws-for-u-backend.onrender.com/delete/${props.pet._id}`,
        {
          method: "DELETE",
        }
      );
